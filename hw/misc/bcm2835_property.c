...
static void bcm2835_property_mbox_push(BCM2835PropertyState *s, uint32_t value)
{
    ...
    while (value + 8 <= s->addr + tot_len) {
        ...
        {
            uint32_t myoffset = ldl_le_phys(&s->dma_as, value + 12);
            uint32_t mylength = ldl_le_phys(&s->dma_as, value + 16);
            int resp;

            if (myoffset > 255 || mylength < 1 || mylength > 256) {
                resp = 1; /* invalid request */
            } else {
                for (uint32_t e = 0; e < mylength; e++) {
                    uint32_t color = ldl_le_phys(&s->dma_as, value + 20 + (e << 2));
                    stl_le_phys(&s->dma_as,
                                s->fbdev->vcram_base + ((myoffset + e) << 2), color);
                }
                resp = 0;
            }
        ...
        case RPI_FWREQ_SET_GPIO_STATE:
            ...
            qemu_log_mask(LOG_UNIMP, "RPI_FWREQ_GET_POWER_STATE "
                          "is not implemented\n");
            break;
